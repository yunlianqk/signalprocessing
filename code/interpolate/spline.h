...
*** 入口参数： (pSPLINE)pLine
样条结构体指针pLine中的x[],y[],num,begin_k1,end_k1 
...
RESULT Spline3(pSPLINE pLine) {
  ...
  for (i = 0; i <= pLine->point_num - 2; i++) { //求H[i]
    H[i] = pLine->x[i + 1] - pLine->x[i];
    Fi[i] = (pLine->y[i + 1] - pLine->y[i]) / H[i]; //求F[x(i),x(i+1)]
  }
  for (i = 1; i <= pLine->point_num - 2; i++) { //求U[i]和A[i]和D[i]
    U[i] = H[i - 1] / (H[i - 1] + H[i]);
    A[i] = H[i] / (H[i - 1] + H[i]);
...
float get_delay(float x1, float y0, float y1, float y2) {
  ...
  for (i = 1; i < 20; i++) {
    j = line1.x[0] + i * 0.05;
    s = line1.a3[0] * pow(line1.x[1] - j, 3) + line1.a1[0] * (line1.x[1] - j) +
        line1.b3[0] * pow(j - line1.x[0], 3) + line1.b1[0] * (j - line1.x[0]);
    if (s > maxvalue) {
      maxvalue = s;
      mp = j;
    }
  ...
  for (i = 1; i < 20; i++) {
    j = line1.x[1] + i * 0.05;
    s = line1.a3[1] * pow(line1.x[2] - j, 3) + line1.a1[1] * (line1.x[2] - j) +
        line1.b3[1] * pow(j - line1.x[1], 3) + line1.b1[1] * (j - line1.x[1]);
    if (s > maxvalue) {
      maxvalue = s;
      mp = j;
    }

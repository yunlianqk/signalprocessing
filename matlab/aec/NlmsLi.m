function [micOut, erle] = NlmsLi(micSpec, spkSpec, nTaps, mu)
...
for (ii = 1:nFrames)
    ...
    spkHistoryPower = spkHistoryPower - spkHistory(:,1).*conj(spkHistory(:,1)) + spkSpec(:,ii).*conj(spkSpec(:,ii));
    ...
    if (sum(spkHistoryPower)/(nBands*nTaps) > adaptSignalThreshold^2)
        ...
        h = h + repmat(Beta, 1, nTaps).* conj(spkHistory);
    end;
    if mod(ii, 100) == 0
        fprintf('%d\n', ii);
    end;

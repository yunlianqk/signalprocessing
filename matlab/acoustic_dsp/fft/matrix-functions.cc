...
#include "mobvoi/matrix/fast_matrix.h"
...
template
void ComplexFft (const VectorBase<float> &in,
                VectorBase<float> *out, bool forward);
template
void ComplexFft (const VectorBase<double> &in,
...
template<class Real> void RealFft (VectorBase<Real> *v, bool forward) {
  ...
  if (!forward) {
    ...
    mobvoi::FastMatrix::ApplyRescale(2.0, v->Dim(), v->Data());
  }
}
...
template<class Real> void ComputeDctMatrix(Matrix<Real>* M) {
  ...
  DCHECK_GT(N, 0);
  // Normalizer for X_0.
  Real normalizer = sqrt(1.0 / static_cast<Real>(N));
  ...
  for (MatrixIndexT k = 1; k < K; k++)
    for (MatrixIndexT n = 0; n < N; n++)
      (*M)(k, n) =
          normalizer * cos( static_cast<double>(M_PI)/N * (n + 0.5) * k );
}

...
%Rife-Vincent(III)-4窗
w=1-1.43596*cos(2*pi*n/N)+0.49754*cos(4*pi*n/N)-0.06158*cos(6*pi*n/N);
x=s.*w;                      % 信号乘以窗函数
v=fft(x,N);                  % FFT
u=abs(v);                    % 取频谱的幅值
k1=zeros(1,M);               % 初始化
...
% RV(III)-4窗的beta对alpha的表示式
    a=-2.681178+6.697249*b-7.016253*b^2+6.115817*b^3-3.914381*b^4+...
        1.674384*b^5-0.423245*b^6+0.047607*b^7;   % rife-vincent
...
% 按式(7-2-9)依RV(III)-4窗的窗函数关系计算出谐波的幅值
    A(i)=yk*(2+0.653039*(gama)^2+0.115798*(gama)^4+0.015691*(gama)^6)/N; % rife-voncent
